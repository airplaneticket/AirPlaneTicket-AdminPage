...
module.exports.addSeat = async(req, res, next) => {
    ...
    let seat = await seatTypeModel.findOne({
        $or: [{ name: inputData.name }, { seatTypeCode: inputData.seatTypeCode }]
    });
    if (seat) {
...
module.exports.saveEditSeat = async(req, res, next) => {
    ...
    let checkSeat = await seatTypeModel.findOne({ name: inputData.name });
    if (checkSeat.seatTypeCode !== seat.seatTypeCode) {
        req.flash("error", "Trùng tên hạng ghế");
        res.redirect("/seat");
        return;
    } else if (seat.name === inputData.name) {
